...
@section('content')
    <div class="col-md-8">
        ...
        <form enctype="multipart/form-data" class="bg-white shadow-sm p-3" action="{{route('users.update', [$user->id])}}" method="POST">
            ...
            <br>
            <input type="checkbox" {{in_array("ADMIN", json_decode($user->roles)) ? "checked" : ""}} class="form-control {{$errors->first('roles') ? "is-invalid" : ""}}" name="roles[]" id="ADMIN" value="ADMIN">
            <label for="ADMIN">Administrator</label>

            <input type="checkbox" {{in_array("STAFF", json_decode($user->roles)) ? "checked" : ""}} class="form-control {{$errors->first('roles') ? "is-invalid" : ""}}" name="roles[]" id="STAFF" value="STAFF">
            <label for="STAFF">Staff</label>

            <input type="checkbox" {{in_array("CUSTOMER", json_decode($user->roles)) ? "checked" : ""}} class="form-control {{$errors->first('roles') ? "is-invalid" : ""}}" name="roles[]" id="CUSTOMER" value="CUSTOMER">
